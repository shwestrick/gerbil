"""Unit tests for the cosmetic tool-call rendering in render.py.

Pure functions, no Docker. render.style() is a no-op off a TTY, so output is plain
...
def test_write_file_line_numbers() -> None:
    out = format_tool_call("write_file", {"path": "A.lean", "content": "a\nb\nc\n"})
...
def test_edit_file_real_line_numbers() -> None:
    ...
    out = format_tool_call("edit_file", args, lambda p: FILE)
...
def test_edit_file_fallback_without_file() -> None:
    ...
    out_none = format_tool_call("edit_file", args, None)
    check("no reader -> fragment line 1", "1 -def foo := 1" in out_none, out_none)

    def boom(_):
        raise FileNotFoundError

    out_boom = format_tool_call("edit_file", args, boom)
...
def test_lean_multi_attempt() -> None:
    ...
    out = format_tool_call("lean_multi_attempt", {
    ...
    out2 = format_tool_call("lean_multi_attempt", {
    ...
    out3 = format_tool_call("lean_multi_attempt", {
...
def test_lean_run_code() -> None:
    ...
    out = format_tool_call("lean_run_code", {"code": code})
    ...
    out2 = format_tool_call("lean_run_code", {"code": big})
    ...
    out3 = format_tool_call("lean_run_code", {"code": "#check Nat"})
...
def test_file_preview_boundary() -> None:
    ...
    out20 = format_tool_call("write_file", {"path": "C.lean", "content": twenty})
    ...
    out21 = format_tool_call("write_file", {"path": "C.lean", "content": twentyone})
...
def test_lean_build() -> None:
    ...
    check("clean flag shown",
          format_tool_call("lean_build", {"clean": True}).strip()
          == "-> lean_build (clean)")
    check("both flags shown",
          format_tool_call("lean_build", {"clean": True, "fetch_cache": True}).strip()
          == "-> lean_build (clean, fetch cache)")


def test_lean_diagnostic_messages() -> None:
    out = format_tool_call("lean_diagnostic_messages", {"file_path": "Foo.lean"})
